...
export async function crearServicio(req, res) {
  ...
  try {
    ...
    const [r] = await pool.execute(
      "INSERT INTO servicios (nombre,descripcion,duracion_minutos,precio,esta_activo,esta_publicado,creado_por,actualizado_por) VALUES (?,?,?,?,?,?,?,?)",
      [
        nombre,
        descripcion ?? null,
        duracion_minutos,
    ...
    const [row] = await pool.execute(
      "SELECT id,nombre,descripcion,duracion_minutos,precio,esta_activo,esta_publicado,creado_en,actualizado_en FROM servicios WHERE id=?",
      [r.insertId]
    );
...
export async function listarServicios(req, res) {
  ...
  const [rows] = await pool.query(
    `SELECT id,nombre,descripcion,duracion_minutos,precio,esta_activo,esta_publicado,creado_en,actualizado_en
...
export async function actualizarServicio(req, res) {
  ...
  try {
    ...
    await pool.execute(
      "UPDATE servicios SET nombre=COALESCE(?,nombre), descripcion=COALESCE(?,descripcion), duracion_minutos=COALESCE(?,duracion_minutos), precio=COALESCE(?,precio), esta_activo=COALESCE(?,esta_activo), esta_publicado=COALESCE(?,esta_publicado), actualizado_por=? WHERE id=?",
      [
        nombre ?? null,
        descripcion ?? null,
...
export async function eliminarServicio(req, res) {
  ...
  try {
    await pool.execute(
      "UPDATE servicios SET esta_activo=0, actualizado_por=? WHERE id=?",
