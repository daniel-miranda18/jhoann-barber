...
export async function crearVenta(req, res) {
  ...
  try {
    const [r] = await pool.execute(
      "INSERT INTO ventas (cliente_id,cajero_id,estado,total,metodo_pago,esta_activo) VALUES (?,?,?,?,?,1)",
      [clienteId, cajeroId, "abierta", 0, null]
    );
...
export async function detalleVenta(req, res) {
  ...
  res.json({ data: { ...v[0], servicios: sv, productos: pv, pagos: pgs } });
}
  ...
  try {
    ...
    await pool.execute(
      `INSERT INTO venta_servicios
       (venta_id,servicio_id,barbero_id,duracion_minutos,precio_unitario,subtotal,esta_activo)
       VALUES (?,?,?,?,?,?,1)`,
      [
        ventaId,
        p.data.servicio_id,
...
export async function eliminarServicioDeVenta(req, res) {
  ...
  await pool.execute(
    "UPDATE venta_servicios SET esta_activo=0 WHERE id=? AND venta_id=?",
    [itemId, ventaId]
  );
  ...
  try {
    ...
    await pool.execute(
      `INSERT INTO venta_productos
       (venta_id,producto_id,cantidad,precio_unitario,subtotal,esta_activo)
       VALUES (?,?,?,?,?,1)`,
      [
        ventaId,
        p.data.producto_id,
...
export async function eliminarProductoDeVenta(req, res) {
  ...
  const itemId = Number(req.params.itemId);
  await pool.execute(
    "UPDATE venta_productos SET esta_activo=0 WHERE id=? AND venta_id=?",
    [itemId, ventaId]
  );
  const tot = await recalcVenta(ventaId);
  res.json({ mensaje: "Eliminado", totales: tot });
...
export async function pagarVenta(req, res) {
  ...
  try {
    ...
    for (const pg of p.data.pagos) {
      await pool.execute(
        "INSERT INTO pagos (venta_id,metodo,monto,referencia,esta_activo) VALUES (?,?,?,?,1)",
        [ventaId, pg.metodo, pg.monto, pg.referencia ?? null]
      );
    }
    ...
    const tot = await recalcVenta(ventaId);
    res.json({ mensaje: "Pagado", totales: tot });
  } catch (e) {
    res.status(400).json({ mensaje: e.message });
  }
}
...
export async function anularVenta(req, res) {
  ...
  try {
    ...
    await conn.execute(
      "UPDATE venta_servicios SET esta_activo=0 WHERE venta_id=?",
      [id]
    );
    await conn.execute(
      "UPDATE venta_productos SET esta_activo=0 WHERE venta_id=?",
      [id]
    );
    await conn.execute("UPDATE pagos SET esta_activo=0 WHERE venta_id=?", [id]);
    await conn.commit();
    conn.release();
...
export async function buscarClientes(req, res) {
  ...
  if (!qtxt) return res.json({ data: [] });
  const [rows] = await pool.query(
    `SELECT id, CONCAT(IFNULL(nombres,''),' ',IFNULL(apellidos,'')) AS nombre, telefono, correo
     FROM clientes
